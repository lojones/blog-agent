...
from agent.data_class.blog_data import BlogOutline, BlogOutlineEvaluation, BlogOutlineSimple   
...
class Evaluator:
    def __init__(self):
        ...
        self.system_prompt_url = "https://github.com/lojones/blog-agent-data/blob/main/system-prompts/evaluate-outline.md"
        self.system_prompt_text = self.github_reader.read_file(self.system_prompt_url)
        self.system_prompt_obj = SystemMessage(content=self.system_prompt_text)
    ...
    def get_initial_prompt(self, outline: BlogOutlineSimple) -> HumanMessage:
    ...
    def get_revision_prompt(self, outline: BlogOutlineSimple) -> HumanMessage:
    ...
    def evaluate_outline(self, outline: BlogOutline) -> BlogOutline:
        ...
        if len(eval.messages) == 0:
            eval.messages = [self.system_prompt_obj, self.get_initial_prompt(outline.outline)]
        else:
            eval.messages.append(self.get_revision_prompt(outline.outline))
